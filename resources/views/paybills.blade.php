        ...
        <div class="col-sm-12 col-md-12">
            <div class="col-md-3">
                <div class="card bg-c-blue order-card">
                    <div class="card-body">
                        <h6 class="m-b-20">Paybills</h6>
                        <h2 class="text-left"><span id="totalAmount">{{ $paybills->count() }}</span></h2>
                    </div>
                </div>
            </div>
            <div> <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#AddpaybillModal">Add
                    paybill</button>

                <div id="AddpaybillModal" class="modal fade" tabindex="-1" role="dialog"
                    aria-labelledby="AddpaybillModalTitle" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="AddpaybillModalTitle">Add Paybill Wallet</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
                            </div>
                            <div class="modal-body">
                                <form method="POST" action="{{ route('create_paybill') }}">
                                    @csrf
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="dt-responsive table-responsive">
                        <table id="paybills-table" class="table  table-striped table-bordered nowrap">
                            <thead>
                                <tr>
                                    <th>Paybill Name</th>
                                    <th>SHORTCODE</th>
                            ...
                            <tbody>
                                @foreach ($paybills as $paybill)
                                    <tr>
                                        ...
                                        <th>{{ $paybill->name }}</th>
                                        <th>{{ $paybill->shortcode }}</th>
                                        <th>{{ $paybill->created_at }}</th>
                                    </tr>
                                @endforeach
                            <tfoot>
                                <th>Username</th>
...
@section('page-js')
    ...
    <script>
        $(document).ready(function() {
            var t = $('#paybills-table').DataTable({
                order: [
                    [0, 'desc']
                ],
            });
